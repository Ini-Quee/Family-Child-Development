...
function verifyAssignmentAccess(assignmentId, familyId) {
...
router.post('/:id/complete', authenticate, (req, res) => {
  try {
    ...
    // IDOR check
    const a = db.prepare(`SELECT ca.*, c.requires_approval, c.xp_value, c.money_value, c.requires_photo, ch.family_id
      FROM chore_assignments ca JOIN chores c ON ca.chore_id = c.id JOIN children ch ON ca.child_id = ch.id
...
router.post('/:id/approve', authenticate, (req, res) => {
  try {
    ...
    // IDOR check
    const a = db.prepare(`SELECT ca.*, c.xp_value, c.money_value, ch.family_id
      FROM chore_assignments ca JOIN chores c ON ca.chore_id = c.id JOIN children ch ON ca.child_id = ch.id
      WHERE ca.id = ?`).get(req.params.id);
    if (!a) return res.status(404).json({ error: 'Assignment not found' });
    if (a.family_id !== req.user.familyId) return res.status(403).json({ error: 'Access denied' });
...
router.post('/:id/reject', authenticate, (req, res) => {
  try {
    ...
    // IDOR check
    const a = db.prepare(`SELECT ca.*, ch.family_id FROM chore_assignments ca JOIN children ch ON ca.child_id = ch.id WHERE ca.id = ?`).get(req.params.id);
    if (!a) return res.status(404).json({ error: 'Assignment not found' });
    if (a.family_id !== req.user.familyId) return res.status(403).json({ error: 'Access denied' });
